...
var gopacJavascriptUtils string = `
...
func gopacIsPlainHostName(host string) bool {
	return strings.Index(host, ".") == -1
}

// dnsDomainIs return true if the host is valid for the domain.
func gopacDnsDomainIs(host, domain string) bool {
...
// matches.
func gopacLocalHostOrDomainIs(host, hostdom string) bool {
	if host == hostdom {
		return true
	}
...
func gopacIsResolvable(host string) bool {
	if len(host) == 0 {
		return false
	}
...
func gopacDnsResolve(host string) string {
	address, err := net.ResolveIPAddr("ip4", host)
